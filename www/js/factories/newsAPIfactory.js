angular.module('isgh.newsAPIfactory', ['isgh.dbAPIfactory'])

    .factory(
    'FactoryNews',
    function($q, $http, Constant, DB, $cordovaSQLite, FactoryProfileLocal) {
        ...
        var _total = function() {
            var query = "SELECT count(id) as total FROM " + table.name;
            return db.query(query).then(function(result) {
                return result;
            }, function(erro) {
                console.log(erro);
    ...
    .factory(
    'FactoryNewsLocal',
    function($q, $http, Constant, FactoryProfileLocal) {
